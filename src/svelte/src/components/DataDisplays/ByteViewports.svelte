<script lang="ts">
  ...
  onMount(() => {
    ...
    physicalContainer = document.getElementById('physical')
    logicalContainer = document.getElementById('logical')
  ...
  $: {
    const totalLines = Math.ceil(byteContent.length / bytesPerLine)
    endLine = startLine + totalLines - 1
  }

  function renderHex(byteArray: Uint8Array): string {
    return Array.from(byteArray)
      .map((byte) => byte.toString(16).padStart(2, '0'))
      .join(' ')
  }

  function renderLatin1(byteArray: Uint8Array): string {
    return Array.from(byteArray)
      .map((byte) => {
