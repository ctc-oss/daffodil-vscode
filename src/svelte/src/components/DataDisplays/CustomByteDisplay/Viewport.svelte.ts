import type { BytesPerRow, ByteValue, RadixValues, ViewportRefreshResponse } from 'ext_types'
import {
    numDisplayLines,
    radixSelections,
} from '../../../stores/format/index.svelte.ts'
import { displaySettings } from 'stores/displaySettings.svelte.ts'
import { byte_value_string, offsetToLineNum, type ViewportLineData } from './DataFeed.svelte.ts'
...
export class ViewportDataState {
    ...
    // })
    private idState = $state<string>('')
    private offsetMaxState = $state(-1)
    private fileOffsetState: number = $state(-1)
    private lengthState: number = $state(-1)
    private capacityState: number = $state(-1)
    private bytesLeftState: number = $state(-1)
    private dataState: Uint8Array = $state(new Uint8Array(0).fill(-1))
    private boundariesState = $derived.by<ViewportFetchBoundary>(() => {
        return {
...
export function getMaxTopLine(viewport: ViewportDataState, numBytesPerRow: BytesPerRow = displaySettings.bytesPerRow) {
