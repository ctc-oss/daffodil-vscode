...
export function isLatin1Editable(char: string): boolean {
...
export function regexEditDataTest(text: string, dataType: string | number): boolean {
  switch(dataType){
    case 2 || 'binary':
...
export function validRequestableData(data: string, viewport: string, encoding: string, editMode: string, radix: number): ValidationResponse {
  switch(editMode) {
    case 'simple':
      if(data.length === 0) return {valid: false, errMsg: ''}
      return (viewport === 'physical')? validEncodingStr(data, radix, editMode) : validEncodingStr(data, 'latin1', editMode)
    case 'full':
