...
/**
 * Retrieves common completion items that apply across multiple element contexts.
 * Filters and creates completion items from the common items intellisense data,
 * including items like xmlns declarations and other frequently used attributes.
 *
 * Common items are attributes or elements that can appear in many different
 * contexts (e.g., xmlns namespace declarations, common XSD attributes like
 * 'name', 'type', 'ref', etc.).
 *
 * @param itemsToUse - Array of item names to include (acts as a filter/whitelist)
 * @param preVal - Prefix value to prepend to each completion snippet (default: '')
 * @param additionalItems - Context-specific string passed to commonCompletion for filtering
 * @param nsPrefix - The namespace prefix being used (e.g., 'xs:', 'xsd:')
 * @returns Array of VS Code completion items for common attributes/elements
 */
export function getCommonItems(
  ...
  // Get common completion data and filter by itemsToUse
