...
const EDITOR_PATH_KEY = (key: string) => {
  return path.resolve(key).toLowerCase()
}
...
export class DataEditorClient implements vscode.Disposable {
  ...
  public async dispose(): Promise<void> {
    ...
    if (OPEN_EDITORS.get(pathKey) === this.panel) {
      OPEN_EDITORS.delete(pathKey)
    }

    try {
      if (this.omegaSessionId.length > 0) {
  ...
  public async initialize(): Promise<void> {
    const pathKey = EDITOR_PATH_KEY(this.fileToEdit)
    const openEditor = OPEN_EDITORS.get(pathKey)
    if (openEditor && openEditor !== this.panel) {
      openEditor.reveal()
      this.panel.dispose()
      return
    }
      ...
      case 'profile':
        {
          ...
          const computedFileSize = this.fileInfoData.computedFileSize
          startOffset = Math.min(Math.max(0, startOffset), computedFileSize)
          length = Math.min(
            length < 0 ? DATA_PROFILE_MAX_LENGTH : Math.max(0, length),
            DATA_PROFILE_MAX_LENGTH,
            computedFileSize - startOffset
          )

          if (length === 0) {
            vscode.window.showErrorMessage(
              `Unable to profile data at offset ${startOffset}: no bytes are available`
            )
            return
          }
...
async function createDataEditorWebviewPanel(
  ...
  const openEditor = OPEN_EDITORS.get(EDITOR_PATH_KEY(fileToEdit))
  if (openEditor) {
    openEditor.reveal()
    return undefined
  }
